...
class GObject:
	...
	def scale(self, sf=None, sx=None, sy=None):
		...
		platform.Platform().scale(self, sx, sy)
	...
	def rotate(self, theta):
		...
		'''
		self.transformed = True
		platform.Platform().rotate(self, sx, sy)
	...
	def contains(self, pt=None, x=None, y=None):
		...
		if(self.transformed):
			return platform.Platform().contains(self, x, y)
...
class GRect(GObject):
	...
	def setSize(self, size=None, width=None, height=None):
		...
		self.width = width
		self.height = height
		platform.Platform().setSize(self, width, height)
...
class GCompound(GObject):
	...
	def contains(self, x, y):
		...
		@rtype: boolean
		'''
		if(self.transformed): return platform.Platform().contains(self, x, y)
		for i in range(len(self.contents)):
	...
	def sendForward(self, gobj):
		...
		if(index != len(self.contents)-1):
			...
			platform.Platform().sendForward(gobj)
	...
	def sendToFront(self, gobj):
		...
		if(index != len(self.contents)-1):
			self.contents.pop(index)
			self.contents.append(gobj)
			platform.Platform().sendToFront(gobj)
	...
	def sendBackward(self, gobj):
		...
		if(index != 0):
			self.contents.pop(index)
			self.contents.insert(index - 1, gobj)
			platform.Platform().sendBackward(gobj)
	...
	def sendToBack(self, gobj):
		...
		if(index != 0):
			...
			platform.Platform().sendToBack(gobj)
...
class GRoundRect(GRect):
	...
	def __init__(self, width, height, x = 0, y = 0, corner = __DEFAULT_CORNER__):
		...
		self.fillFlag = False
		self.fillColor = ""
		platform.Platform().createGRoundRect(self, width, height, corner) 
		self.setLocation(x=x, y=y)
...
class G3DRect(GRect):
	...
	def __init__(self, width, height, x = 0, y = 0, raised = False):
		...
		self.fillFlag = False
		self.fillColor = ""
		platform.Platform().createG3DRect(self, width, height, str(raised).lower()) 
		self.setLocation(x=x, y=y)
...
class GOval(GObject):
	...
	def __init__(self, width, height, x=0, y=0):
		...
		self.fillFlag = False
		self.fillColor = ""
		platform.Platform().createGOval(self, width, height)
		self.setLocation(x=x, y=y)
...
class GArc(GObject):
	...
	def setStartAngle(self, start):
		...
		'''
		self.start = start
		platform.Platform().setStartAngle(self, start) 
	...
	def setSweepAngle(self, sweep):
		...
		'''
		self.sweep = sweep
		platform.Platform().setSweepAngle(self, sweep) 
	...
	def getFrameRectangle(self):
		...
		@rtype: GRectangle
		'''
		return gtypes.GRectangle(0,0,0,0) #!!
	...
	def contains(self, x, y):
		...
		return self.containsAngle(math.atan2(-dy, dx) * 180 / gmath.PI)
	...
	def getArcPoint(self, theta):
		...
		return gtypes.GPoint(cx + rx * math.cos(radians), cy - ry * math.sin(radians))
...
class GLine(GObject):
	...
	def __init__(self, x0, y0, x1, y1):
		...
		self.dx = x1 - x0
		self.dy = y1 - y0
		platform.Platform().createGLine(self, x0, y0, x1, y1) 
	...
	def setStartPoint(self, x, y):
		...
		self.x = x
		self.y = y
		platform.Platform().setStartPoint(self, x, y) 
	...
	def setEndPoint(self, x, y):
		...
		self.dx = x - self.x
		self.dy = y - self.y
		platform.Platform().setEndPoint(self, x, y) 
...
class GImage(GObject):
	...
	def __init__(self, filename, x=0, y=0):
		...
		size = platform.Platform().createGImage(self, filename) 
...
class GLabel(GObject):
	...
	def __init__(self, str, x=0, y=0):
		...
		GObject.__init__(self)
		self.str = str
		platform.Platform().createGLabel(self, str) 
		self.setFont(__DEFAULT_GLABEL_FONT__)
		size = platform.Platform().getGLabelSize(self) 
		self.width = size.getWidth()
		self.height = size.getHeight()
		self.ascent = platform.Platform().getFontAscent(self) 
		self.descent = platform.Platform().getFontDescent(self) 
		self.setLocation(x=x, y=y)
	...
	def setFont(self, font):
		...
		'''
		self.font = font
		platform.Platform().setFont(self, font) 
		size = platform.Platform().getGLabelSize(self)
		self.width = size.getWidth()
		self.height = size.getHeight()
		self.ascent = platform.Platform().getFontAscent(self) 
	...
	def getBounds(self):
		...
		'''
		if(self.transformed): return platform.Platform().getBounds(self) 
		return gtypes.GRectangle(self.x, self.y - self.ascent, self.width, self.height)
...
class GPolygon(GObject):
	...
	def __init__(self):
		...
		self.cy = None
		self.vertices = []
		platform.Platform().createGPolygon(self) 
	...
	def addPolarEdge(self, r, theta):
		...
		@rtype: void
		'''
		self.addEdge(r*math.cos(theta*gmath.PI/180), \
					-r*math.sin(theta*gmath.PI/180)) 
