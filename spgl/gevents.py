'''
This file defines the even types used in the Stanford Portable Graphics Libraries. 
The structure of this package is adapted from the Java event model.

EventClassType
==============
This enumeration definest the vent classes. The element values are each a single bit 
...
def getNextEvent(mask = EventClassType.ANY_EVENT):
	...
	@rtype: GEvent
	'''
	return platform.Platform().getNextEvent(mask)
