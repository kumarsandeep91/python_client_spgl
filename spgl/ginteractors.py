...
class GInteractor(gobjects.GObject):
	...
	def setActionCommand(self, cmd):
		...
		platform.Platform().setActionCommand(self, cmd) 
	...
	def getBounds(self):
		...
		return gtypes.GRectangle(self.x, self.y, size.getWidth(), size.getHeight())
...
class GButton(GInteractor):
	...
	def __init__(self, label):
		...
		self.label = label
		platform.Platform().createGButton(self, label) 
...
class GCheckBox(GInteractor):
	...
	def __init__(self, label):
		...
		GInteractor.__init__(self)
		self.label = label
		platform.Platform().createGCheckBox(self, label) 
...
class GSlider(GInteractor):
	...
	def __init__(self, min=0, max=100, value=50):
		...
		self.min = min
		self.max = max
		platform.Platform().createGSlider(self, min, max, value) 
...
class GTextField(GInteractor):
	...
	def __init__(self, nChars=10):
		...
		'''
		GInteractor.__init__(self)
		platform.Platform().createGTextField(self, nChars) 
...
class GChooser(GInteractor):
	...
	def __init__(self):
		...
		'''
		GInteractor.__init__(self)
		platform.Platform().createGChooser(self) 
